...
- 답: 1->A, B, C의 커밋이 그대로 main 브랜치로 병합, 2->A, B, C의 커밋을 하나의 커밋으로 main 브랜치로 병합, 3->모두 새로 커밋으로 변경
...
- 답: 현재 작업중인 위치: develop, develop에 readme를 추가함
...
- 답: git reset --soft 345hj26, soft 옵션
...
- 답: revert. reset는 commit를 삭제하므로 위험하기 때문에 생성하여 되돌리는 revert가 적절하다.
